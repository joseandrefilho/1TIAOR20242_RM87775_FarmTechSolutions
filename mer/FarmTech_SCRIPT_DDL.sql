...
ALTER TABLE t_cultura ADD CHECK ( vl_area_cultivo > 0 );
...
ALTER TABLE t_irrigacao ADD CHECK ( vl_quantidade_agua_aplicada > 0 );
...
COMMENT ON COLUMN t_sensor.nm_sensor IS
...
ALTER TABLE t_sensor ADD CONSTRAINT un_t_sensor_nm_sensor UNIQUE ( nm_sensor );
...
COMMENT ON COLUMN t_tipo_produto.nm_tipo_produto IS
    'Descrição do tipo de produto.';

COMMENT ON COLUMN t_tipo_produto.nm_unidade_medida IS
